...
export const PermissionDetailUserPart = (props: IPermissionDetailUserPartProps) => {
    ...
    const editerUserList = selectUserList?.filter(u => u._role.includes(UnitRole.Editor)) ?? [];
    ...
    const handleAddPerson = async (type: 'edit' | 'view' = 'edit') => {
        const userList = await authzIoService.listCollaborators({
            objectID: unitId,
            unitID: unitId,
        });

        sheetPermissionUserManagerService.setAuthzType(type)

        const currentUser = userManagerService.getCurrentUser();
        sheetPermissionUserManagerService.setCanEditUserList(
            userList.filter((user) =>
                user.subject?.userID !== currentUser.userID
            )
        );
        dialogService.open({
    ...
    useEffect(() => {
        const getSelectUserList = async () => {
            ...
            const selectUserList = collaborators.filter(user => user._role?.length > 0);
        ...
        <>
            ...
            {editState === EditStateEnum.DesignedUserCanEdit && (
                ...
                >
                    <div className="univer-flex univer-items-center univer-justify-between univer-text-sm">
                        ...
                        <span className="univer-cursor-pointer univer-text-primary-600" onClick={() => handleAddPerson('edit')}>{localeService.t('permission.panel.addPerson')}</span>
                    </div>
                    <div className="univer-my-2 univer-h-px univer-bg-gray-200" />
                    <div className="univer-flex-1">
                        {editerUserList?.length > 0
                            ? editerUserList.map((item) => {
                                return (
                                    <div
            ...
            <FormLayout className="univer-font-medium" label={localeService.t('permission.panel.viewPermission')}>
                ...
                >
                    ...
                    <Radio value={'appointedUserCanView'}>
                        {/* <span>{localeService.t('permission.panel.noOneElseCanView')}</span> */}
                        <span>指定用户</span>
                    </Radio>
                </RadioGroup>
            </FormLayout>
            {viewState === 'appointedUserCanView' && (
                <div
                    className={clsx(`
                      univer-mb-2 univer-flex univer-h-[270px] univer-flex-col univer-overflow-y-auto univer-rounded-lg
                      univer-p-3
                ...
                >
                    <div className="univer-flex univer-items-center univer-justify-between univer-text-sm">
                        <span>{localeService.t('permission.panel.designedPerson')}</span>
                        <span className="univer-cursor-pointer univer-text-primary-600" onClick={() => handleAddPerson('view')}>{localeService.t('permission.panel.addPerson')}</span>
                    </div>
                    <div className="univer-my-2 univer-h-px univer-bg-gray-200" />
                    <div className="univer-flex-1">
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <Select
                                            className="!univer-w-[90px] univer-min-w-0 univer-cursor-pointer"
                                            borderless
                                            value={item._role.includes(UnitRole.Reader) ? 'able' : 'disabled'}
                                            options={[
                                                { label: `可查看`, value: 'able' },
                                                { label: `不可查看`, value: 'disabled' },
                                                { label: `${localeService.t('permission.panel.delete')}`, value: 'delete' },
                                            ]}
                                            onChange={(v) => {
                                                if (v === 'delete') {
                                                    ...
                                                    item._role = v === 'able' ? [UnitRole.Reader] : [UnitRole.UNRECOGNIZED];
                                                    sheetPermissionUserManagerService.setSelectUserList([...selectUserList]);
                                                }
                                            }}
                                        />
